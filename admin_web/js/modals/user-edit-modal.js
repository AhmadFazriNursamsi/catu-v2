              ...
              <form id="editUserForm" onsubmit="handleSaveProfile(event)" class="space-y-5">
                ...
                <div class="bg-slate-50/80 p-5 rounded-2xl border border-slate-200/80 space-y-4 shadow-xs">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select id="editAccountStatus" onchange="syncEditFormToState(); state.activeEditUser.account_status = this.value;" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl font-black text-slate-900 text-xs focus:outline-none focus:ring-2 focus:ring-amber-500 shadow-xs transition">
                ...
                ${roleCode === 'ROMO_PAROKI' ? (() => {
                  ...
                  <div class="bg-blue-50/40 p-5 rounded-2xl border border-blue-100/80 space-y-4 shadow-xs">
                    ...
                    <div class="grid grid-cols-1 ${isK ? 'md:grid-cols-3' : ''} gap-4">
                ...
                ${roleCode === 'ROMO_ORDO' ? (() => {
                  ...
                  <div class="bg-blue-50/40 p-5 rounded-2xl border border-blue-100/80 space-y-4 shadow-xs">
                    ...
                    <div class="grid grid-cols-1 ${isKetuaOrdo ? 'md:grid-cols-3' : ''} gap-4">
                ...
                <div class="pt-5 border-t border-slate-200 flex items-center justify-end space-x-3">
                  <button type="button" onclick="state.activeEditUser = null; renderApp();" class="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs transition">Batal</button>
                  <button type="submit" ${state.isSavingProfile ? 'disabled' : ''} class="px-6 py-2.5 rounded-xl bg-blue-950 hover:bg-blue-900 text-white font-bold text-xs flex items-center space-x-2 shadow-xs transition disabled:opacity-50">
                    ${state.isSavingProfile ? `<span class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span><span>Menyimpan Perubahan...</span>` : `<i data-lucide="save" class="w-4 h-4"></i><span>Simpan Perubahan</span>`}
                  </button>
                </div>
              </form>
            </div>
